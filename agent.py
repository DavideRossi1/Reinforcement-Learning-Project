...
class Agent():
    ...
    def side_position_binning_boost(self,env):
        pol_near=(env.length-env.polposl<2*env.car_length) # cars are almost at the same height, they could crash from now on
        wall_left= 0 if (env.carposw < C.SPEED) else \
                   1 if (env.carposw<2*C.SPEED) else \
                   2 if (env.carposw<2*C.BOOST*C.SPEED) else \
                   3
        right_car_side=env.carposw+env.car_width
        wall_right=0 if (env.width-right_car_side < C.SPEED) else \
                   1 if (env.width-right_car_side<2*C.SPEED) else \
                   2 if (env.width-right_car_side<2*C.BOOST*C.SPEED) else \
                   3
                   
        left_dist= (env.carposw-env.polposw)%env.width-env.car_width
        right_dist=(env.polposw-env.carposw)%env.width-env.car_width
        pol_left=  0 if (pol_near and (left_dist < C.SPEED)) else \
                   1 if (pol_near and (left_dist<2*C.SPEED)) else \
                   2 if (pol_near and (left_dist<2*C.BOOST*C.SPEED)) else \
                   3
        pol_right= 0 if (pol_near and (right_dist < C.SPEED)) else \
                   1 if (pol_near and (right_dist<2*C.SPEED)) else \
                   2 if (pol_near and (right_dist<2*C.BOOST*C.SPEED)) else \
                   3
        obs_left= min(wall_left, pol_left)
        obs_right=min(wall_right,pol_right)
        pos= [pol_left,pol_right] if C.PACMAN else [obs_left,obs_right]
    ...
    def front_position_binning(self,env):
        left_dist= (env.carposw-env.polposw)%env.width
        right_dist=(env.polposw-env.carposw)%env.width
        pol_in_front=(min(left_dist,right_dist) <env.car_width)
        ...
        pol_dist= 0 if (pol_in_front and (car_vertdist < C.SPEED)) else \
                  ...
                  2 if (pol_in_front and (car_vertdist<env.car_width//(C.BOOST*C.SPEED)+c)) else \
                  3 if (pol_in_front and (car_vertdist<env.car_width//C.SPEED+c)) else \
                  4
        pol_leftright = (min(left_dist,right_dist)==left_dist) #(env.polposw<env.carposw)  # police car on the left of the car if true, on the right (possibly center) if false
